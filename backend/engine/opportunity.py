"""Opportunity Engine — LLM-powered opportunity reasoning for BuilderDNA 2.0.
...
def format_trends_for_llm(snapshot: TrendSnapshot) -> str:
    ...
    """
    lines = []
    topics = snapshot.topics[:5]  # Max 5 topics

    for topic in topics:
        topic_name = topic.topic
        stage = topic.stage
        velocity = topic.growth_velocity
        top_repos = topic.top_repos[:3]  # Top 3 repos
...
def format_pains_for_llm(snapshot: PainSnapshot) -> str:
    ...
    """
    lines = []
    clusters = snapshot.clusters[:5]  # Max 5 pain clusters

    for cluster in clusters:
        title = cluster.title
        severity = cluster.severity
        description = cluster.description or ""
        affected_repos = cluster.affected_repos[:3]  # Top 3 affected repos
...
"""
    ...
    try:
        ...
        # Fallback: return empty list on LLM error
        return []
...
async def run_opportunity_engine(
    ...
    """Run full opportunity engine pipeline.
